# Modified from https://github.com/wesselvanree/LCR-Rot-hop-ont-plus-plus
...
class EmbeddingsDatasetCustom(Dataset):
    def __init__(self, year: int, phase='Train', domain="Restaurants", ont_hops: Optional[int] = None, device=torch.device('cpu'),
                 empty_ok=False, enable_cache=True, use_vm=True, use_soft_pos=True):
        # Modified directory naming to use format: {year}-{phase}-{domain}
        self.dir = f'data/embeddings/{year}-{phase}-{domain}'
    ...
    def __repr__(self):
        return f"EmbeddingsDatasetCustom({self.dir})"


def train_validation_split(dataset: EmbeddingsDatasetCustom, validation_size=0.2, seed: Optional[float] = None):
